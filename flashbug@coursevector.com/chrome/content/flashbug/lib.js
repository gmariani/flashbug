				...
				if (xulString) {
					...
					if(element.localName == 'p') {
						...
						try {
							...
							var injectHTML = _htmlSvc.parseFragment(localized, false, null, element); 
							element.appendChild(injectHTML);
						}
					} else {
		...
		saveMMFile: function(mm) {
			...
			try {
				...
				if (_prefSvc.getBoolPref('extensions.firebug.flbInspector.enableSites') == true) {
					...
					if (mm['PreloadSwf'] == path) mm['PreloadSwf'] = '';
				}
