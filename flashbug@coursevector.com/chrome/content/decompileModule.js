...
function(Obj, Domplate, Events, Options, Locale, FBTrace, Str, Url, Xpcom, Win, Http, NetUtils, Dom, IO) {
...
var childPanelName = "flashDecompilerTree";
...
Flashbug.DecompileModule = Obj.extend(Firebug.ActivableModule, {
	...
	*/
	initialize: function(prefDomain, prefNames) {
		...
		//Firebug.NetCacheReader.addListener(this);
	...
	*/
	shutdown: function() {
		...
		//Firebug.NetCacheReader.removeListener(this);
    },
	...
	*/
	...
	showPanel: function(browser, panel) {
		...
		//Dom.collapse(Firebug.chrome.$("fbFlashbugDecompileButtons"), !isPanel);
        Dom.collapse(Firebug.chrome.$("fbFlashbugVersion"), !isPanel);
		
		if (isPanel) {
			var side = Firebug.currentContext.getPanel(childPanelName);
	...
	dispatchName: 'flashbug.decPanel.title',
	description: 'flashbug.decPanel.description',
	...
	onResponse: function(context, file) {
		if (!file) return;
		trace("onResponse " + file.href + ' ' + file.size);
		this.addSWF(context, file, file.request.URI.asciiSpec);
	},
	
	// Initial
	/*onExamineCachedResponse: function(context, request) {
		//trace("onExamineCachedResponse " + file.request.URI.asciiSpec);
		this.addSWF(context, request, request.URI.asciiSpec);
	},*/
	...
	onCachedResponse: function(context, file) {
		if (!file) return;
		trace("onCachedResponse " + file.href + ' ' + file.size);
		this.addSWF(context, file, file.request.URI.asciiSpec);
	},
	
	addSWF: function(context, file, href) {
		//trace('addSWF() ' + file.mimeType, file);
		...
		//trace('addSWF mimeType: ' + mimeType);
		var tabId = Win.getTabIdForWindow(context.window);
		...
		// If is a SWF
		if (mimeType == Flashbug.XSWF_MIME || mimeType == Flashbug.SWF_MIME || mimeType == Flashbug.SPL_MIME) {
			var hasAdded = false,
			...
			if (!context2.swfs[href]) {
				...
				// File has finished loading, update SWF title
				if (context instanceof Firebug.TabContext) {
					context.invalidatePanels(panelName);
					//context.getPanel(childPanelName).refresh();
				}
			}
	...
	Flashbug.DecompileModule.SWF = domplate(Firebug.Rep, {
		...
		getFileSize: function(obj) {
			...
			if (value.hasOwnProperty('svgHeader')) {
				...
				return Str.formatSize(data.length);
			}
			if (value.hasOwnProperty('data')) return Str.formatSize(value.data.length);
			return Str.formatSize(value.dataSize);
		},
		...
		getParamValueIterator: function(param) {
			...
			return Str.wrapText(param, true);
		},
		...
		onSave: function(event) {
			...
			if (file) IO.writeFile(file, data);
			...
			DIV({"class": "flb-dec-InfoBody"},
				...
				TABLE({cellpadding: 0, cellspacing: 0},
					TBODY({"class": "netInfoResponseHeadersBody"},
						...
						TR({},
							TD({'class': 'netInfoParamName'}, SPAN(Locale.$STR('flashbug.netInfoSWF.dimensions'))),
							TD({'class': 'netInfoParamValue'},
		...
		onLoadImage: function(event) {
			...
			if (obj.alphaData && !obj.hasAlpha) {
				...
				var uri = canvas.toDataURL();//,
					//timeout = Xpcom.CCIN('@mozilla.org/timer;1', 'nsITimer');
...
Firebug.registerRep(
	Flashbug.DecompileModule.SWF, 
	Flashbug.DecompileModule.Binary,
	Flashbug.DecompileModule.PixelBender,
	Flashbug.DecompileModule.Bitmap,
	Flashbug.DecompileModule.Video,
	Flashbug.DecompileModule.Font,
	Flashbug.DecompileModule.Shape,
	Flashbug.DecompileModule.MorphShape,
	Flashbug.DecompileModule.Sound,
	Flashbug.DecompileModule.StreamSound,
	Flashbug.DecompileModule.ActionScript,
	Flashbug.DecompileModule.TextField,
	Flashbug.DecompileModule.Header,
	Flashbug.DecompileModule.XML
);
Firebug.registerActivableModule(Flashbug.DecompileModule);

return Flashbug.DecompileModule;
